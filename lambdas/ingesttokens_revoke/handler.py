...
================================================================================
Self-serve foundation Phase 3. A Cognito-authed caller revokes an ingest token
...
ownerId (Cognito sub): a user can only revoke their own token. Revoking a token
...
def handler(event: dict, context: Any) -> dict:
    ...
    if not owner_id:
        raise AuthorizationError(
            message="Caller has no Cognito sub; cannot revoke an ingest token.",
            handler=HANDLER,
            function="handler",
        )
