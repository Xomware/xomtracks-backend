...
XOMTRACKS Logger
================
Centralized logging for all Lambda functions. Ported from xomify-backend's
lambdas/common/logger.py (see docs/features/xomtracks/PLAN.md).

- No duplicate log entries (handler added once per Lambda container)
- Consistent formatting across all functions
- Context-aware logging with module info
"""
...
class XomtracksLogger:
    ...
    def __init__(self, log_level: str = "INFO"):
        ...
        if XomtracksLogger._initialized and XomtracksLogger._logger:
            self.logger = XomtracksLogger._logger
            return

        self.log_level = log_level.upper()

        self.logger = logging.getLogger("xomtracks")
        ...
        XomtracksLogger._logger = self.logger
        XomtracksLogger._initialized = True
...
LOGGER = XomtracksLogger(LOG_LEVEL)
